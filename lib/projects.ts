...
export async function getProjectById(id: string, includeDraft = false): Promise<Project | null> {
  ...
  let query = getProjectsDb().from('projects').select('*').eq('id', id).maybeSingle();
  if (!includeDraft) query = query.eq('published', true);
  const { data, error } = await query;
  if (error) throw error;
  return data ? fromDb(data as DbProject) : null;
}
